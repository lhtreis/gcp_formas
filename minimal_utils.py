...
def loss_func_mod(probs, adj_tensor):
    ...
    :return: Loss, given the current soft assicondagnments (probabilities)
    :rtype: float
    """
...
def loss_func_color_hard(coloring, nx_graph, adj_tensor):
    """
    ...
    for (u, v) in nx_graph.edges:
        cost_ += 1*(coloring[u] == coloring[v])*(u != v)#*adj_tensor[u][v]
